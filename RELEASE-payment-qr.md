...
- Existing `/upi-qr.jpeg` WhatsApp links remain valid. The URL redirects to the backend's repository-owned PNG response; the legacy URL extension is retained for compatibility.
...
2. Deploy the backend first, then the frontend. Confirm the Render static-site redirect `/upi-qr.jpeg` points to `https://semiskitchenbe.onrender.com/api/payment-qr/image` ahead of the SPA fallback. Do not add a public frontend file at that path, which could shadow the redirect.
