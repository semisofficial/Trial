...
Optional Playwright: run `node test/browser-menu-inventory.mjs` against local Vite on port 5174; run `node test/browser-security-headers.mjs` after building. Set `PLAYWRIGHT_MODULE` to your Playwright installation when necessary. Both scripts intercept external/API traffic; neither places live orders. The latter serves built files with the checked-in CSP.
